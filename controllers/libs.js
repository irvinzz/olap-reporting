...
var md5 = require('../public/js/libs/md5.js');
var entrySet = {
    server: {
...
function getRandKey(l, cm) { // l - length,  cm - usable characters
    var l = l || 32;
    var keyChars = (cm || '0123456789abcdef').split('');
    var key = '';
    for (var i = 0; i < l; ++i) {
...
module.exports.paloPool = {};
/*
function getPool(req,res){
    pCI.getall({
        prefix: 'PaloServers'
    },function(err,result){
        async.map(result,function(el,cb){
            console.log('connecting to ',el);
            FGetClient({
                host: el.ipaddress,
                port: el.port,
                user: el.login,
                extern_password: el.password
            },function(err,result){
                console.log('bingo');
                console.log(err);
                console.log(result);
...
function getPaloClient(opts,cb){
    if (pool[opts.key]===undefined){
        pool[opts.key]={
            state: 'connecting'
        };
    }
    FGetClient({
        ...
        port: opts.port,
        user: opts.user,
        password: opts.password || md5(opts.extern_password)
    },function(err,result){
        if (err) {
            delete pool[opts.key];
            return cb(err,null);
        }
        pool[opts.key]=result;
        pool[opts.key].state = 'connected';
        setTimeout(function(){ delete pool[opts.key] },+result.ttl*1000);            //  kill client after tll expires, 
        (function _(){
            setTimeout(function(){
                if (--result.ttl>0){ _() }
            },1000);
...
function getPaloClientByIndex(index,cb){
    console.log('>>> receiving client by index: ',index);
    if (pool[index]!==undefined){
        return cb (null,pool[index]);
    }else{
        pCI.getbyid({
            id: index,
            prefix: 'PaloServers'
...
var FGetClient = function(opts,cb){
    opts = opts || {};
    var pi = new paloClass({
        host: opts.host,
        port: opts.port
    });
    pi.call('server', 'login', {
        query: {
            user: opts.user,
            password: opts.password || undefined,
            extern_password: opts.extern_password || undefined
        }
    }, function(err, result) {
        if (!err) {
            pi.sid = result.rows[0].sid;
            pi.ttl = result.rows[0].ttl;
            cb(null, pi);
        }
        else {
            cb(err, result);
...
var getGlobalClient = function(opts, cb) {
    getPaloClient({
        key: 'Global',
        host: nconf.get('palo:host'),
        port: nconf.get('palo:port'),
...
module.exports.getPaloClientByIndex = getPaloClientByIndex;
